...
-- [MODIFIED] Fungsi bypass printer hold — tidak nunggu HoldDuration
local function BypassPrintPrompt(prompt)
    ...
    -- Bikin prompt lebih gampang di-trigger
    pcall(function()
        prompt.Enabled = true
    ...
    -- Kalau prompt selesai, langsung stop loop
    pcall(function()
        table.insert(conns, prompt.Triggered:Connect(function()
            finish()
        end))
        table.insert(conns, prompt.TriggerEnded:Connect(function()
            finish()
        end))
    end)
    ...
    task.spawn(function()
        ...
        while not finished and State.IsOfficeActive and tick() < timeout do
            ...
            -- Fallback kalau executor tidak punya fireproximityprompt
            ...
            if not activePrinterName or not prompt.Enabled then
                finish()
                break
            end
    ...
    -- Tunggu sampai benar-benar selesai / printer clear
    local timeout = tick() + 10

    while not finished and State.IsOfficeActive and tick() < timeout do
        if not activePrinterName or not prompt.Parent then
            finish()
            break
        end
        task.wait(0.03)
        ...
        if activePrinterName and not getgenv().isGoingToPrinter then
            ...
            pcall(function()
                ...
                if printerPart and targetPrompt then
                    ...
                    -- [MODIFIED] Ganti wait random + eksekusiPromptTahan jadi bypass
                    task.wait(0.03)
                    BypassPrintPrompt(targetPrompt)
                    ...
                    -- [MODIFIED] Wait lebih cepat, tetap tunggu clear dari server
                    local timeout = 0
                    while activePrinterName and timeout < 8 and State.IsOfficeActive do
                        task.wait(0.1)
                        timeout += 0.1
                    end
                end
