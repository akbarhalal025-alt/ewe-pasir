...
[+] Version     : DDS GLOBAL EDITION (v8.3.2 - F9 CLEAN + NO-TP SIT FIX)
[+] Changelog   : - [FIX] NO-TP SIT: TP hanya 1x saat awal start Office
- [FIX] Setelah print / soal / anti-stuck: jalan ke kursi, maju-mundur sampai duduk
...
-- ============================================================================
-- // 13. OFFICE JOB SYSTEM (v8.3.2 - NO-TP SIT FIX)
-- ============================================================================
...
-- ============================================================================
-- // [FIX] NO-TP SIT SYSTEM
-- // TP + seat:Sit HANYA 1x saat awal start Office (initialSitTPUsed).
-- // Setelah itu: jalan ke kursi, kalau belum duduk maju-mundur sampai duduk.
-- ============================================================================
local initialSitTPUsed = false
local function paksaDudukJalan(chair, maxTry)
maxTry = maxTry or 35
local hum = CharRef.Humanoid
local root = CharRef.Root
...
if not chair then
chair = findOfficeSeat(nil)
end
if chair and isChairOccupied(chair) then
local replacement = findOfficeSeat(chair)
if replacement then chair = replacement end
end
if not chair then return false end
local seat = getSeatFromChair(chair)
-- Kalau bukan Seat biasa, coba pakai ProximityPrompt
if not seat then
local prompt = chair:FindFirstChildWhichIsA("ProximityPrompt")
if prompt then
local pos = nil
if chair:IsA("Model") then
pos = chair:GetPivot().Position
elseif chair:IsA("BasePart") then
pos = chair.Position
end
if pos then
jalanKe(pos + Vector3.new(0, 2, 0))
eksekusiPromptTahan(prompt)
task.wait(0.8)
...
-- Pastikan seat bisa disentuh / diduduki
pcall(function()
seat.Disabled = false
seat.CanTouch = true
...
for i = 1, maxTry do
if hum.SeatPart then return true end
if not State.IsOfficeActive then return false end
if not root.Parent then return false end
local flatRoot = Vector3.new(root.Position.X, 0, root.Position.Z)
local flatSeat = Vector3.new(seat.Position.X, 0, seat.Position.Z)
local toSeat = flatSeat - flatRoot
local dist = toSeat.Magnitude
local dir = (dist > 0.25) and toSeat.Unit or root.CFrame.LookVector
-- Kalau masih jauh, jalan normal ke kursi
if dist > 7 then
jalanKe(seat.Position + Vector3.new(0, 2, 0))
else
hum:MoveTo(seat.Position)
end
task.wait(0.18)
if hum.SeatPart then return true end
-- Maju sedikit ke arah kursi
hum:MoveTo(root.Position + dir * 1.05)
task.wait(0.16)
if hum.SeatPart then return true end
-- Mundur sedikit kalau belum duduk
hum:MoveTo(root.Position - dir * 1.2)
task.wait(0.16)
if hum.SeatPart then return true end
-- Geser kiri/kanan supaya tidak nyangkut di pinggir kursi
local right = root.CFrame.RightVector
hum:MoveTo(root.Position + right * 0.85)
task.wait(0.14)
if hum.SeatPart then return true end
hum:MoveTo(root.Position - right * 0.85)
task.wait(0.14)
if hum.SeatPart then return true end
-- Kadang nyangkut, lompat kecil tiap beberapa percobaan
if i % 6 == 0 then
hum.Jump = true
task.wait(0.2)
end
...
-- ================================================================
-- TP HANYA SEKALI UNTUK AWAL OFFICE
-- ================================================================
local allowInitialTP = instantTP and not initialSitTPUsed
if allowInitialTP then
initialSitTPUsed = true
keluarKursi()
...
or (myChair:IsA("Model") and myChair.PrimaryPart and myChair.PrimaryPart.CFrame)
...
-- ================================================================
-- SEMUA KONDISI SETELAH AWAL: JALAN + MAJU/MUNDUR SAMPAI DUDUK
-- ================================================================
local ok = paksaDudukJalan(myChair)
if ok then return true end
-- Kalau masih gagal, coba kursi lain sekali
local newChair = findOfficeSeat(myChair)
if newChair and newChair ~= myChair then
myChair = newChair
return paksaDudukJalan(myChair)
end
return false
end
-- ============================================================================
...
-- [FIX] Pastikan duduk sebelum jawab — kalau berdiri, dudukkan dulu (JALAN, bukan TP)
...
local hum = CharRef.Humanoid
if hum and not hum.SeatPart then
-- Masih berdiri, coba dudukkan dulu (JALAN, bukan TP), jangan ganti kursi
dudukKeKursi(false)
lastActivityTime = tick()
else
...
-- [FIX] Setelah ngeprint: JALAN ke kursi + maju/mundur sampai duduk (NO TP)
...
Content  = "Fix: NO-TP sit! TP hanya awal, setelah print/soal jalan + maju-mundur sampai duduk!",
Duration = 5,
})
